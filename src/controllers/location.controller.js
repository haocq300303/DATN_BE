...
export const getAll = async (req, res) => {
    try {
        ...
        res.status(200).json(successfully(location, "Lấy dữ liệu thành công"));
    } catch (error) {
...
export const getById = async (req, res) => {
    try {
        ...
        const pitch = await pitchService.getAllPitch({ location_id: req.params.id });
        const data = {
            location: {
                ...location.toObject(),
                pitchs: pitch,
...
export const update = async (req, res) => {
    try {
        ...
        const location = await locationService.updateLocation({ ...req.body, id: req.params.id });
        if (!location) {
