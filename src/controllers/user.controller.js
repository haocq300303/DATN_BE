...
export const getList = async (req, res) => {
    try {
        const { _sort = "createdAt", page = 1, limit = 50, _order = "desc", ...params } = req.query;
        const options = {
            skip: (page - 1) * limit,
            limit,
            sort: {
        ...
        const [users, count] = await Promise.all([userService.getList(options), userService.countDocuments()]);
...
export const loginWithGoogle = async (req, res) => {
    try {
        ...
        const role = await roleService.getById(checkUser.role_id || "655b87021ac3962a68ccf1b5");
        if (checkUser) {
            const values = {
                _id: checkUser._id,
                name: checkUser.name,
...
export const register = async (req, res) => {
    try {
        ...
        const { email, password, name, role_id, phone_number } = req.body;
        // check name
        ...
        const values = {
            ...
            createAt: newUser.createdAt,
            updatedAt: newUser.updatedAt,
            role_name: "655b87021ac3962a68ccf1b5",
        };
        const token = await generateToken(values);
        res.status(200).json(successfully({ ...newUser?.toObject(), token }, "Thêm thành công !!!"));
    } catch (error) {
        res.status(500).json(serverError(error.message));
    }
};
